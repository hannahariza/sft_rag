...
def load_png_images_recursive(image_folder):
    """
    递归查找指定文件夹及其所有子文件夹中的PNG图片
    """
    png_files = []
    ...
    all_images = {}
    for image_id, png_file in tqdm(enumerate(png_files), total=len(png_files), desc="loading images recursively"):
        image = Image.open(png_file)
        all_images[image_id] = image
    return all_images

def get_png_files_recursive(image_folder):
    """
    递归获取指定文件夹及其所有子文件夹中的PNG图片文件路径列表
    """
    png_files = []
    for root, dirs, files in os.walk(image_folder):
        for file in files:
